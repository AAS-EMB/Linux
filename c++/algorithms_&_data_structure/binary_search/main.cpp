...
int main(int argc, char *argv[]) {
  ...
  for (auto Iterator = &Array[0]; Iterator != (&Array[0] + ArraySize);
       Iterator++) {
    std::cin >> *Iterator;
  }
  ...
  if (SearchMemoryAddress != nullptr) {
    std::cout << "BinarySearch_1: for number " << SearchValue
              << " memory address " << SearchMemoryAddress << std::endl;
  } else {
    std::cout << "BinarySearch_1: " << SearchValue
              << " was not found in the list" << std::endl;
  }
  int SearchIndex = BinarySearch_2(Array, ArraySize, SearchValue);
  if (SearchIndex != -1) {
    std::cout << "BinarySearch_2: for number " << SearchValue << " index "
              << SearchIndex << std::endl;
  } else {
    std::cout << "BinarySearch_2: " << SearchValue
              << " was not found in the list" << std::endl;
  }
  delete[] Array;
  return 0;
...
int *BinarySearch_1(int *pArray, unsigned int ArraySize, int SearchValue) {
  ...
  while (FirstIndex <= LastIndex) {
    ...
    if (*AverageIndex == SearchValue)
      return AverageIndex;
    *AverageIndex > SearchValue ? LastIndex = AverageIndex - 1
...
int BinarySearch_2(int *pArray, unsigned int ArraySize, int SearchValue) {
  ...
  while (FirstIndex <= LastIndex) {
    ...
    if (pArray[AverageIndex] == SearchValue)
      return AverageIndex;
    pArray[AverageIndex] > SearchValue ? LastIndex = AverageIndex - 1
                                       : FirstIndex = AverageIndex + 1;
  }
  return -1;
}
