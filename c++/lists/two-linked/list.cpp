...
void List::AddNewList(unsigned int NumberList)
{
	...
	pTempList_2->pPrevList = pTempList_1;
...
void List::RemoveSingleList(unsigned int NumberList)
{
	...
	List_t *pPrevList = nullptr, *pCurrList = nullptr, *pNextList = nullptr;
	...
	while(CurrentIndex != NumberList)
	{
		...
		pNextList = pCurrList->pNextList;
		CurrentIndex++;
	}
	pPrevList->pNextList = pCurrList->pNextList;
	if(pNextList != nullptr)
		pNextList->pPrevList = pCurrList->pPrevList;
	delete pCurrList;
}
...
void List::PrintAllList(void)
{
	...
	{
		std::cout << "Current list: " << pTempList->CurrentList + 1 << "\t\tPrev address list: " << pTempList->pPrevList << "\t\tCurrent address list: " << pTempList << "\t\tNext address list: " << pTempList->pNextList << std::endl;
...
void List::PrintSingleList(unsigned int NumberList)
{
	...
	std::cout << "Number list: " << pTempList->CurrentList + 1 << "\t\tPrev address list: " << pTempList->pPrevList << "\t\tCurrent address list: " << pTempList << "\t\tNext address list: " << pTempList->pNextList << std::endl;
}